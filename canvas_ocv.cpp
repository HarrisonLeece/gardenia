...
#include <exception>
#include <limits> // Used for cin check
  ...
  for (int count = 0; cap.get(CAP_PROP_FRAME_COUNT); count++ ) {
    ...
    try {
      if (frame.channels()==4) {
        ...
        imwrite(output_path+"output"+to_string(count)+".png", frame);
...
void superimpose(int frameNumber_in, vector<vizModule> vizModules, Mat bck, int frameRate)
{
  try {
    ...
    imwrite("./composite_pngs/composited_img" + to_string(frameNumber_in) + ".png", bckg);
    cout << "Frame " << to_string(frameNumber_in) << " superimposed." << endl;
  } catch (exception& e) {
...
void vCanvas::renderCanvas(){
  ...
  cout << "Enter the number of frames in this render (integer): " << endl;
  int input1;
  cin >> input1;
  ...
  while (!cin.good())
  {
      ...
      cout << "Format incorrect, enter only an integer" << endl;
      cout << "Enter the number of frames in this render (integer): " << endl;
      cin >> input1;
  }

  cout << "Read: " << input1 << endl;
  ...
  cout << "Convert video AND standardize the names of files in directories?  Enter 0 to skip, otherwise enter 1:" << endl;
  int input2 = -1;
  cin >> input2;
  while (input2 != 0 && input2 != 1)
  {
      cin.clear();
      cin.ignore(numeric_limits<streamsize>::max(), '\n');
      cout << "Format incorrect, enter only 0 (do not convert) or 1 (convert)" << endl;
      cin >> input2;
  }
